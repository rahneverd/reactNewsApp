...
export class News extends Component {
  ...
  async componentDidMount() {
    ...
    if(parsedData.articles.length % 3) {
      ...
      this.setState({articles : parsedData.articles.slice(0, -remainder), pageSize: Math.floor(parsedData.totalResults / 18)<6?Math.floor(parsedData.totalResults / 18):5, loading: false})
    } else {
      this.setState({articles : parsedData.articles, pageSize: Math.floor(parsedData.totalResults / 18)<6?Math.floor(parsedData.totalResults / 18):5, loading: false})
    }
